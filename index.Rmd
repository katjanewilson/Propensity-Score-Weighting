---
title: "Achieving Balance through Weighting"
output:
  html_document:
   theme: united
   toc: true
...
These observable characteristics are crucial to achieving a balanced treatment and control sample. But a tradeoff of simple matching is that the study sample is now a fraction of the data. The ATT is also only internally valid for the matched sample, and the higher probability that some subjects have of being in the treatment group over others might be important yet ignored. The propensity score - the probability that the subject is in the treatment group or control- are full of information themselves. Propensity scores are not dependent on the treatment group, but rather dependent on the covariates’ relationship to the treatment. Weighting, therefore, is a method to assign a higher weight to subjects who are not likely to be in the treatment class, giving these units more information. Subjects who are not likely to be in the treatment are rare, and valuable. Units who are more likely to be in the treatment have repeated information and are down-weighted. In the same spirit of the manipulation that underlies much of causal inference, weighting helps creates a more balanced “pseudo-population” to proceed with a balanced analysis.


Such weighting of the propensity score is directly parallel to the Horvitz Thompson estimator (Horvitz and Thompson, 1952) in survey literature. The Horvitz Thompson estimator performs inverse probability weighting (gives each observation a weight which is the inverse of its probability of inclusion), and provides an unbiased estimator of the population total and population mean under unequal probability sampling. Using such survey weights, the estimate for the population mean becomes



$$\hat{y}HT = \frac{1}{N}\sum\limits_{i=1}^{n} w_i y_i $$

Similarly, Inverse Probability Treatment Weighting (Rosenbaum 1987), assigns a higher weight to those units which are less likely to be included, just as units underrepresented in the sample compared to the population, are assigned a higher weight in the Horvitz-Thompson approach. A suite of balance diagnostics to assess balance between groups has been proposed by Austin and Stuart (Austin & Stuart 2015). The formula for the IPTW weights is below, where each subject is weighted by the inverse of their treatment probability.
$$
p(x)=P(Z=1 \mid X) \\
\begin{equation}
  \hat{w}(IPTW) =
...
A second type of propensity score matching method is Standardized IP-weighting. Standardized weighting deals with the issue in IPTW weighting where individuals with PS close to 0 (those extremely unlikely to be treated) will end up with a very large weight, making the estimator unstable. Standardized weights use the marginal probability of treatment instead of 1 in the weight numerator. This example is parallel to standardized weights in survey literature, where weighting techniques like standardized weighting are used to address issues with low response rates, caused by survey coverage and unit nonresponse. 

In the worked example that will follow, we have a population of schools which are matched on a set of five observable covariates. In this case, a very small number of schools (high poverty, low-minority schools) are unlikely to be in the treatment. When these schools do appear in the treatment, we want to give their information as much weight as possible. IPTW helps to retain most of the individuals in the analysis, increasing the effective sample size. And in another similarity with survey literature, as I will note in the worked example, weighting uses the survey package directly.
...
## Part 2: Propensity Score Matching and Nearest Neighbor Matching

Nearest neighbor matching improves balance on all five covariates. Store model results.
```{r message=FALSE, warning=FALSE, include=TRUE}
...
One concern regarding propensity score weighting is that observations with extremely large weights may over-influence results and yield estimates with high variance. In that case, a common recommendation is to trim observations with large survey weights. The suggestion of weight trimming also comes directly from survey design methods (Potter 1993), where weight trimming has been shown to reduce the sampling variance estimate. The tradeoffs suggested from the survey literature parallel the tradeoffs we will see for weight trimming in PS methods: larger sample variance or substantial bias might result for some survey estimates, despite the overall decrease of variance.
...
A limitation, as with all other PS methods, is that researchers are at the whim of which covariates you include in your model. Additionally, the ATE is now in reference to the target population, which is the overlap population. The estimation of the treatment could be for a subpopulation that does not reflect people who receive the treatment in routine care. In our NYC example, we might be estimating people who have no chance of the treatment.
...
### then use the PSweight function to get ATE for IPW
...
### then use the PSweight function to get ATE for Overlap
ate.any <- PSweight(ps.formula = ps.formula,
                    yname = "Y", data = data1,
                    weight= "overlap")
# estimated average potential outcomes for the treatment
