...
These observable characteristics are crucial to achieving a balanced treatment and control sample. But a tradeoff of simple matching is that the study sample is now a fraction of the data. Additionally, the ATT is, under these methods, only internally valid for the matched sample. Even more, the higher probability that some subjects have of being in the treatment group over others might be important, yet is ignored. 

However, the propensity scores - the probability that the subject is in the treatment group or control- have crucial information which might curtail some of these concerns. Propensity scores are not dependent on the treatment group, but rather are dependent on the covariates’ relationship to the treatment. Weighting is a method from the statistical survey literature whereby higher weights are assigned to subjects who are not likely to be in the treatment class. Using propensity scores, this method can be applied in the context of an observational study. Treated subjects who are not likely to be in the treatment are rare, and therefore have valuable information. Treated subjects more likely to be in the treatment have repeated information, and are down-weighted. In the same spirit of the manipulation that underlies much of causal inference, weighting helps creates a more balanced “pseudo-population” by which we can proceed with a balanced analysis.
...
In the worked example that will follow, a population of schools in NYC are matched on a set of five observable covariates. In this case, a very small number of schools (high poverty, low-minority schools) are unlikely to be in the treatment. When these schools do appear in the treatment, we want to give their information as much weight as possible. IPTW helps to retain most of the individuals in the analysis, increasing the effective sample size. 
...
Logistic regression to predict treatment by five covariates creates the propensity score. Matching on propensity score does little to improve covariate balance. Nearest neighbor will do more.
```{r message=FALSE, warning=FALSE, include=TRUE}
...
Nearest neighbor matching matches each unit with nearest neighbor in a 4:1 ratio of control to treatment units. Nearest neighbor matching improves balance for all five covariates, and crosses 0.1 standardized mean difference threshold.
